function [jmp] = p1fluxjmps(p1sol, eex, xy, evt, eboundt, tve, s, coeff_fun)
...
[~,invjac,~,dphidx,dphidy] = tderiv(s,1-s,xl_v,yl_v);
[coeff] = tgauss(s, 1-s, xl_v, yl_v, coeff_fun);
% first PHYSICAL edge
hx_v = xl_v(:,3) - xl_v(:,2);
hy_v = yl_v(:,3) - yl_v(:,2);
...
fx_v = zero_v;
for ivtx = 1:3
    fx_v = fx_v + sl_v(:,ivtx).*( coeff(:).*dphidx(:,ivtx).*nx_v + ...
        coeff(:).*dphidy(:,ivtx).*ny_v ).*invjac(:);
end
flux(:,1) = fx_v;


% second REFERENCE edge - left
[~,invjac,~,dphidx,dphidy] = tderiv(0,s,xl_v,yl_v);
[coeff] = tgauss(0, s, xl_v, yl_v, coeff_fun);
...
[~,invjac,~,dphidx,dphidy] = tderiv(s,0,xl_v,yl_v);
[coeff] = tgauss(s, 0, xl_v, yl_v, coeff_fun);
% third PHYSICAL edge
hx_v = xl_v(:,2) - xl_v(:,1);
hy_v = yl_v(:,2) - yl_v(:,1);
he_v = sqrt(hx_v.*hx_v + hy_v.*hy_v);
