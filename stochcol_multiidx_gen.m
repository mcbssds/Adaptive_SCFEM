function MULTI_IDX = stochcol_multiidx_gen(N,rule,w,base,multiidx,MULTI_IDX)
%STOCHCOL_MULTIIIDX_GEN multi-index generation
%
% MULTI_IDX = multiidx_gen(N,rule,w,base,[],[])
%
% computes multi indexes M_I of length N with elements such that rule(M_I) <= w.
...
if length(multiidx)~=N 
      ...
      while rule([multiidx, i]) <= w
            ...
            MULTI_IDX = stochcol_multiidx_gen(N,rule,w,base,[multiidx, i],MULTI_IDX);
            i=i+1;
      end
