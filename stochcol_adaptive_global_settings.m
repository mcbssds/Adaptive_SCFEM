...
% AB: the lines below (if--end) were modified to choose the precomputation data depending on rule_id
% existence of the file is NOT checked (next line)
% if exist('precomputation.mat', 'file') == 2 % FX: check existence of the file precomputation.mat
if rule_id == 1
    load precomputation_leja.mat
    if max_level_p == max_level && rule_id_p == rule_id && rv_id_p == rv_id && L_p == L
        ...
        rv_id_p = rv_id;
        L_p = L;
        save precomputation_leja.mat max_level_p rule_id_p rv_id_p fun_p polys L_p list
    end
elseif rule_id == 2
    load precomputation_cc.mat
    if max_level_p == max_level && rule_id_p == rule_id && rv_id_p == rv_id && L_p == L
    else
        polys = stochcol_onedlagpolys(max_level, rule_id);
        list = stochcol_uni_int(fun_p, polys, L);
        ...
        rv_id_p = rv_id;
        L_p = L;
        save precomputation_cc.mat max_level_p rule_id_p rv_id_p fun_p polys L_p list
    end
end
